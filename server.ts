...
const INITIAL_DATA = {
  ...
  socialMessages: [
    {
      user: "Coach Mike",
      msg: "Team Blue got lucky last night. We coming for that rematch!",
      time: "2h ago",
    },
    {
      user: "J-Buckets",
      msg: "Who wants to guard me next Friday? 🏀💨",
      time: "5h ago",
    },
  ],
  socialPosts: [
...
async function startServer() {
  ...
  app.post("/api/reserve", async (req, res) => {
    ...
    const isAlreadyReserved = (data.upcomingGame.reservedPlayers || []).some(
      (p: any) =>
        `${p.firstName} ${p.lastName}`.trim().toLowerCase() === fullName,
    );
    const isAlreadyPendingRes = (
      data.upcomingGame.pendingReservations || []
    ).some(
      (p: any) =>
        `${p.firstName} ${p.lastName}`.trim().toLowerCase() === fullName,
    );
    const isAlreadyPendingPay = (data.upcomingGame.pendingPayments || []).some(
      (p: any) =>
        `${p.firstName} ${p.lastName}`.trim().toLowerCase() === fullName,
    );
  ...
  app.post("/api/admin/confirm-reservation", async (req, res) => {
    const { id } = req.body;
    const data: any = await readData();
    const reservationIndex = data.upcomingGame.pendingReservations.findIndex(
      (r: any) => r.id === id,
    );

    if (reservationIndex > -1) {
      const reservation =
        data.upcomingGame.pendingReservations[reservationIndex];
      ...
      data.upcomingGame.filledSlots = (
        data.upcomingGame.reservedPlayers || []
      ).length;

      const playerExists = data.players.some(
  ...
  app.post("/api/admin/confirm-payment", async (req, res) => {
    const { id } = req.body;
    const data: any = await readData();
    const paymentIndex = data.upcomingGame.pendingPayments.findIndex(
      (p: any) => p.id === id,
    );
    ...
    if (paymentIndex > -1) {
      ...
      const fullNameForGlobal =
        `${payment.firstName} ${payment.lastName}`.trim();
      const playerExists = data.players.some(
        (p: any) => p.name.toLowerCase() === fullNameForGlobal.toLowerCase(),
      );
