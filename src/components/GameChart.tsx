...
export const GameChart: React.FC<GameChartProps> = ({ games }) => {
              ...
              >
                <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
                  ...
                  <div className="flex items-center gap-4 md:gap-8">
                    <div className="text-center">
                      <div className="text-[10px] text-white/40 uppercase mb-1">White</div>
                    ...
                    <div className="text-center">
                      <div className="text-[10px] text-white/40 uppercase mb-1">Blue</div>
                ...
                <div className="flex items-center gap-4">
                  <div className="hidden md:flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full border border-white/10">
                    ...
                    <span className="text-xs font-mono">MVP: {game.mvpId}</span>
                  </div>
                        ...
                        <div className="space-y-2">
                          {(game.teamWhite.players || []).map((p, i) => (
                            <div key={i} className="flex justify-between items-center p-2 bg-white/5 rounded border border-white/5">
                              ...
                              <span className="font-mono text-neon-blue font-bold">{p.pts} PTS</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {(game.teamBlue.players || []).map((p, i) => (
                            <div key={i} className="flex justify-between items-center p-2 bg-white/5 rounded border border-white/5">
                              <span className="text-sm font-medium">{p.name}</span>
                              <span className="font-mono text-neon-red font-bold">{p.pts} PTS</span>
                            </div>
                          ))}
                        </div>
                      </div>
