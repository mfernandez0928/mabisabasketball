...
export const MVPSpotlight: React.FC<MVPSpotlightProps> = ({ player, description }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `MVP_${player.name}.png`, { type: 'image/png' });
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        link.href = dataUrl;
        link.click();
        alert('Image downloaded! You can now upload this picture directly to your Facebook or Instagram story.');
      }
    } catch (err) {
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            >
              ...
              <div className="relative h-full bg-card-bg border border-white/10 rounded-3xl overflow-hidden flex flex-col">
                ...
                <div className="p-6 flex justify-between items-start">
                  <div>
                    <div className="text-neon-blue font-mono text-sm tracking-tighter">KING OF THE COURT</div>
                ...
                <div className="p-6 grid grid-cols-3 gap-4 border-t border-white/10 bg-black/40 backdrop-blur-md">
                  <div className="text-center">
                    <div className="text-2xl font-display text-neon-blue">{player.points}</div>
                    <div className="text-[10px] uppercase text-white/40 tracking-widest">PTS</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-display text-white">{player.rebounds}</div>
                    <div className="text-[10px] uppercase text-white/40 tracking-widest">REB</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-display text-neon-red">{player.assists}</div>
                    <div className="text-[10px] uppercase text-white/40 tracking-widest">AST</div>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <h3 className="text-3xl font-display text-neon-blue">Dominance on Display</h3>
              <p className="text-white/70 leading-relaxed">
                {description || `${player.name} absolutely took over the last run. With a clinical performance from the perimeter and lockdown defense, he led Team Blue to a decisive victory.`}
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex items-center gap-4">
                <div className="p-2 bg-yellow-500/20 rounded-lg"><Trophy className="text-yellow-500" size={20} /></div>
                <div>
                  <div className="text-xs text-white/40 uppercase">Total MVPs</div>
                  <div className="font-bold">{player.mvps}</div>
                </div>
              </div>
              <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex items-center gap-4">
                <div className="p-2 bg-neon-blue/20 rounded-lg"><Zap className="text-neon-blue" size={20} /></div>
                <div>
