    ...
    <section className="py-20 px-4">
      <div className="max-w-6xl mx-auto space-y-20">
        <div className="text-center space-y-4">
          <h2 className="text-5xl md:text-7xl grungy-text italic">The Spotlight</h2>
          <p className="text-white/40 font-mono text-sm uppercase tracking-widest">Honoring the best from the last run</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {awards.map((award, index) => (
            <div key={award.id || `${award.type}-${index}`} className="space-y-6">
              {/* Poster Container */}
              <div 
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-t ${award.type === 'player_of_the_night' ? 'from-neon-blue/40' : 'from-neon-red/40'} via-transparent to-transparent`} />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <img 
                    src={award.photoUrl || 'https://picsum.photos/seed/player/800/1000'} 
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-between">
                  ...
                  <div className="flex justify-between items-start">
                    <div className={`px-4 py-2 rounded-lg font-display text-xl skew-x-[-12deg] ${award.type === 'player_of_the_night' ? 'bg-neon-blue text-black' : 'bg-neon-red text-white'}`}>
                      {award.type === 'player_of_the_night' ? 'PLAYER OF THE NIGHT' : 'HUSTLE PLAYER'}
                    </div>
                    <div className="bg-black/80 backdrop-blur-md border border-white/10 p-2 rounded-lg">
                      {award.type === 'player_of_the_night' ? <Trophy className="text-neon-blue" /> : <Zap className="text-neon-red" />}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="flex-1">
                        <h3 className="text-6xl md:text-8xl font-display leading-none tracking-tighter uppercase break-words">
                          {award.playerName.split(' ').map((part, i) => (
                            <span key={i} className={i % 2 === 0 ? 'text-white' : award.type === 'player_of_the_night' ? 'text-neon-blue' : 'text-neon-red'}>
                              {part}<br />
                            </span>
                          ))}
                        </h3>
                    ...
                    <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/5">
                      <p className="text-sm text-white/80 italic leading-snug">"{award.caption}"</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 right-4 opacity-50">
                  <div className="text-[8px] font-mono text-white/40 uppercase tracking-[0.3em]">Mabisa Basketball Club</div>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                >
                  <Download size={20} className="text-neon-blue group-hover:scale-110 transition-transform" />
                  <span className="text-xs uppercase tracking-widest font-bold">Export Poster</span>
                </button>
