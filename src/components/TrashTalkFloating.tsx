...
export const TrashTalkFloating: React.FC<TrashTalkFloatingProps> = ({ messages }) => {
    ...
    <div className="fixed bottom-32 md:bottom-6 right-6 z-[100] flex flex-col items-end">
          ...
          >
            ...
            <div className="p-3 flex items-center justify-between border-b border-white/10 bg-neon-red/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider">Trash Talk Corner</span>
              </div>
                ...
                >
                  {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-2">
                      ...
                      <p className="text-[10px] font-mono text-white/20 uppercase">No banter yet. Be the first to talk trash!</p>
                    </div>
                  ) : (
                    messages.map((chat, i) => (
                      <div key={i} className="space-y-1">
                        <div className="flex justify-between items-center px-1">
                          <span className="text-[10px] font-bold text-neon-blue uppercase">{chat.user}</span>
                          <span className="text-[9px] text-white/20">
                            {chat.timestamp?.toDate ? chat.timestamp.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Just now"}
                          </span>
                        </div>
      ...
      <motion.button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${
          isOpen ? 'bg-neon-red text-white rotate-90' : 'bg-neon-blue text-black'
        }`}
      >
